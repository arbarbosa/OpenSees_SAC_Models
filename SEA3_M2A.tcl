...
## INPUTs
set Qake "acelerograma.g3";
set Qake_factor "$g*1.";
set npts_accelerogram 3000;   
set dt_accelerogram 2.0E-02;
...
set Weight_Piso1 [expr 4.705E6*$N]; 		# superstructure weight
set Weight_Piso2 [expr 4.705E6*$N]; 
set Weight_Piso3 [expr 5.090E6*$N]; 
...
element truss	30041	4105	5110	1 100;
...
element truss	30042	4205	5210	1 100;
...
element truss	30043	4305	5310	1 100;
...
# define material properties
	...
	set G [expr 89*$GPa];
	set Fy_column [expr 344.14*$MPa];
	set Fy_beam [expr 344.14*$MPa];
	...
	set IDSteel_column_153 2
	uniaxialMaterial Steel02 $IDSteel_column_153 [expr 1.53*$Fy_column] [expr 1.74*$Es] $Bs  $R0 $cR1 $cR2;

	set IDSteel_column_158 3
	uniaxialMaterial Steel02 $IDSteel_column_158 [expr 1.58*$Fy_column] [expr 1.84*$Es] $Bs  $R0 $cR1 $cR2;

	set IDSteel_column_25 4
	uniaxialMaterial Steel02 $IDSteel_column_25 [expr 2.5*$Fy_column] [expr 2.5*$Es] $Bs  $R0 $cR1 $cR2;


# define column sections - distributed plasticity with fiber-sections
set W14x159 159;
set d [expr 14.98*$in];
set bf [expr 15.565*$in];
set tf [expr 1.19*$in];
set tw [expr 0.745*$in];
...
Wsection  $W14x159 $IDSteel_column $d $bf $tf $tw $nfdw $nftw $nfbf $nftf

set W14x176 176;
set d [expr 15.22*$in];
set bf [expr 15.65*$in];
set tf [expr 1.31*$in];
set tw [expr 0.83*$in];
set nfdw 8;		# number of fibers along dw
set nftw 3;		# number of fibers along tw
set nfbf 8;		# number of fibers along bf
set nftf 3;			# number of fibers along tf
Wsection  $W14x176 $IDSteel_column $d $bf $tf $tw $nfdw $nftw $nfbf $nftf

set W14x176_CF1_WA 17601; #WEAK AXIS
	set dw [expr $d - 2 * $tf]
	set y1 [expr -$d/2]
	set y2 [expr -$dw/2]
	set y3 [expr  $dw/2]
	...
	section fiberSec  $W14x176_CF1_WA  {
   	patch quadr  $IDSteel_column_158  $nfbf $nftf   $z1 $y1   $z4 $y1   $z4 $y2   $z1 $y2
   	patch quadr  $IDSteel_column_158  $nftw $nfdw   $z2 $y2   $z3 $y2   $z3 $y3   $z2 $y3
   	patch quadr  $IDSteel_column_158  $nfbf $nftf   $z1 $y3   $z4 $y3   $z4 $y4   $z1 $y4
	}
	
set W14x176_CF2_WA 17602; #WEAK AXIS
	section fiberSec  $W14x176_CF2_WA  {
   	patch quadr  $IDSteel_column_153  $nfbf $nftf   $z1 $y1   $z4 $y1   $z4 $y2   $z1 $y2
   	patch quadr  $IDSteel_column_153  $nftw $nfdw   $z2 $y2   $z3 $y2   $z3 $y3   $z2 $y3
   	patch quadr  $IDSteel_column_153  $nfbf $nftf   $z1 $y3   $z4 $y3   $z4 $y4   $z1 $y4
	}
	
set W10x77_CF 771;
set d [expr 10.6*$in];
set bf [expr 10.19*$in];
set tf [expr 0.87*$in];
set tw [expr 0.53*$in];
set nfdw 8;		# number of fibers along dw
set nftw 3;		# number of fibers along tw
set nfbf 8;		# number of fibers along bf
set nftf 3;			# number of fibers along tf
Wsection  $W10x77_CF $IDSteel_column_25 $d $bf $tf $tw $nfdw $nftw $nfbf $nftf

set W10x60_CF 601;
set d [expr 10.22*$in];
set bf [expr 10.08*$in];
set tf [expr 0.68*$in];
set tw [expr 0.42*$in];
set nfdw 8;		# number of fibers along dw
set nftw 3;		# number of fibers along tw
set nfbf 8;		# number of fibers along bf
set nftf 3;			# number of fibers along tf
Wsection  $W10x60_CF $IDSteel_column_25 $d $bf $tf $tw $nfdw $nftw $nfbf $nftf
...
set tf [expr 0.68*$in];
set tw [expr 0.44*$in];
set I [expr 1.62*2100*$in4];
set A [expr 22.4*$in2];
set My [expr 200*$in3*$Fy_beam];
set curvW24x76 [expr $My / ($Es* $I)];
#Lignos parameters
set LS [expr 0.889/$Lp_Beam];			# basic strength deterioration (a very large # = no cyclic deterioration)
set LK [expr 0.889/$Lp_Beam];			# unloading stiffness deterioration (a very large # = no cyclic deterioration)
set LA [expr 0.889/$Lp_Beam];			# accelerated reloading stiffness deterioration (a very large # = no cyclic deterioration)
set LD [expr 0.889/$Lp_Beam];			# post-capping strength deterioration (a very large # = no deterioration)
...
set th_pP [expr 0.019/$Lp_Beam];		# plastic rot capacity for pos loading
set th_pN [expr 0.019/$Lp_Beam];		# plastic rot capacity for neg loading
set th_pcP [expr 0.116/$Lp_Beam];			# post-capping rot capacity for pos loading
set th_pcN [expr 0.116/$Lp_Beam];			# post-capping rot capacity for neg loading
...
uniaxialMaterial Elastic [expr 200*$W24x76] [expr $Es*$A]   ;#elastic material to describe axial behavior
uniaxialMaterial Elastic [expr 300*$W24x76] [expr $G*$A]  ;#elastic material to describe shear behavior
#without deterioration
#uniaxialMaterial Steel01 [expr 400*$W24x76] [expr 1.0*$My] $Kspr [expr $strhard] ;
#with deterioration
uniaxialMaterial Bilin02  [expr 400*$W24x76] $Kspr $strhard $strhard [expr 1.0*$My] [expr -1.0*$My] $LS $LK $LA $LD $cS $cK $cA $cD $th_pP $th_pN $th_pcP $th_pcN $ResP $ResN $th_uP $th_uN $DP $DN;
#sections for plastic hinges and middle element
section Aggregator $W24x76 [expr 200*$W24x76] P [expr 300*$W24x76] Vy [expr 400*$W24x76] Mz ;  # aggregate axial. shear and flexural behavior
section Elastic [expr 500*$W24x76] $Es $A $I  ;#elastic section to be assigned to the middle of the element
...
set tf [expr 0.77*$in];
set tw [expr 0.47*$in];
set I [expr 1.59*2370*$in4];
set A [expr 24.7*$in2];
set My [expr 224*$in3*$Fy_beam];
set curvW24x84 [expr $My / ($Es* $I)];
#Lignos parameters
set LS [expr 1.029/$Lp_Beam];			# basic strength deterioration (a very large # = no cyclic deterioration)
set LK [expr 1.029/$Lp_Beam];			# unloading stiffness deterioration (a very large # = no cyclic deterioration)
set LA [expr 1.029/$Lp_Beam];			# accelerated reloading stiffness deterioration (a very large # = no cyclic deterioration)
set LD [expr 1.029/$Lp_Beam];			# post-capping strength deterioration (a very large # = no deterioration)
set cS 1.0;				# exponent for basic strength deterioration (c = 1.0 for no deterioration)
set cK 1.0;				# exponent for unloading stiffness deterioration (c = 1.0 for no deterioration)
set cA 1.0;				# exponent for accelerated reloading stiffness deterioration (c = 1.0 for no deterioration)
set cD 1.0;				# exponent for post-capping strength deterioration (c = 1.0 for no deterioration)
set th_pP [expr 0.021/$Lp_Beam];		# plastic rot capacity for pos loading
set th_pN [expr 0.021/$Lp_Beam];		# plastic rot capacity for neg loading
set th_pcP [expr 0.131/$Lp_Beam];			# post-capping rot capacity for pos loading
set th_pcN [expr 0.131/$Lp_Beam];			# post-capping rot capacity for neg loading
set ResP [expr 0.4];			# residual strength ratio for pos loading
set ResN [expr 0.4];			# residual strength ratio for neg loading
set th_uP [expr 0.4/$Lp_Beam];			# ultimate rot capacity for pos loading
set th_uN [expr 0.4/$Lp_Beam];			# ultimate rot capacity for neg loading
...
set strhard [expr (1.0*$My*(1.1 - 1.0)) / ($Kspr*$th_pP)];
#elastic material for axial and shear behavior
uniaxialMaterial Elastic [expr 200*$W24x84] [expr $Es*$A]   ;#elastic material to describe axial behavior
uniaxialMaterial Elastic [expr 300*$W24x84] [expr $G*$A]  ;#elastic material to describe shear behavior
#without deterioration
#uniaxialMaterial Steel01 [expr 400*$W24x84] [expr 1.0*$My] $Kspr [expr $strhard] ;
#with deterioration
uniaxialMaterial Bilin02  [expr 400*$W24x84] $Kspr $strhard $strhard [expr 1.0*$My] [expr -1.0*$My] $LS $LK $LA $LD $cS $cK $cA $cD $th_pP $th_pN $th_pcP $th_pcN $ResP $ResN $th_uP $th_uN $DP $DN;
#sections for plastic hinges and middle element
section Aggregator $W24x84 [expr 200*$W24x84] P [expr 300*$W24x84] Vy [expr 400*$W24x84] Mz ;  # aggregate axial. shear and flexural behavior
section Elastic [expr 500*$W24x84] $Es $A $I  ;#elastic section to be assigned to the middle of the element


set W18x40 40;
set d [expr 17.9*$in];
set bf [expr 6.02*$in];
set tf [expr 0.525*$in];
set tw [expr 0.315*$in];
set I [expr 1.77*612*$in4];
set A [expr 11.8*$in2];
set My [expr 78.4*$in3*$Fy_beam];
set curvW18x40 [expr $My / ($Es* $I)];
#Lignos parameters
set LS [expr 0.852/$Lp_Beam];			# basic strength deterioration (a very large # = no cyclic deterioration)
set LK [expr 0.852/$Lp_Beam];			# unloading stiffness deterioration (a very large # = no cyclic deterioration)
set LA [expr 0.852/$Lp_Beam];			# accelerated reloading stiffness deterioration (a very large # = no cyclic deterioration)
set LD [expr 0.852/$Lp_Beam];			# post-capping strength deterioration (a very large # = no deterioration)
set cS 1.0;				# exponent for basic strength deterioration (c = 1.0 for no deterioration)
set cK 1.0;				# exponent for unloading stiffness deterioration (c = 1.0 for no deterioration)
set cA 1.0;				# exponent for accelerated reloading stiffness deterioration (c = 1.0 for no deterioration)
set cD 1.0;				# exponent for post-capping strength deterioration (c = 1.0 for no deterioration)
set th_pP [expr 0.030/$Lp_Beam];		# plastic rot capacity for pos loading
set th_pN [expr 0.030/$Lp_Beam];		# plastic rot capacity for neg loading
set th_pcP [expr 0.149/$Lp_Beam];			# post-capping rot capacity for pos loading
set th_pcN [expr 0.149/$Lp_Beam];			# post-capping rot capacity for neg loading
set ResP [expr 0.4];			# residual strength ratio for pos loading
set ResN [expr 0.4];			# residual strength ratio for neg loading
set th_uP [expr 0.4/$Lp_Beam];			# ultimate rot capacity for pos loading
set th_uN [expr 0.4/$Lp_Beam];			# ultimate rot capacity for neg loading
...
set strhard [expr (1.0*$My*(1.1 - 1.0)) / ($Kspr*$th_pP)];
#elastic material for axial and shear behavior
uniaxialMaterial Elastic [expr 200*$W18x40] [expr $Es*$A]   ;#elastic material to describe axial behavior
uniaxialMaterial Elastic [expr 300*$W18x40] [expr $G*$A]  ;#elastic material to describe shear behavior
#without deterioration
#uniaxialMaterial Steel01 [expr 400*$W18x40] [expr 1.0*$My] $Kspr [expr $strhard] ;
#with deterioration
uniaxialMaterial Bilin02  [expr 400*$W18x40] $Kspr $strhard $strhard [expr 1.0*$My] [expr -1.0*$My] $LS $LK $LA $LD $cS $cK $cA $cD $th_pP $th_pN $th_pcP $th_pcN $ResP $ResN $th_uP $th_uN $DP $DN;
#sections for plastic hinges and middle element
section Aggregator $W18x40 [expr 200*$W18x40] P [expr 300*$W18x40] Vy [expr 400*$W18x40] Mz ;  # aggregate axial. shear and flexural behavior
section Elastic [expr 500*$W18x40] $Es $A $I  ;#elastic section to be assigned to the middle of the element
...
element	forceBeamColumn	40011	710008	116	$TransfTag	Lobatto	$W14x159	$numIntgrPts	
element	forceBeamColumn	40021	720008	216	$TransfTag	Lobatto	$W14x176	$numIntgrPts	
element	forceBeamColumn	40031	730008	316	$TransfTag	Lobatto	$W14x176	$numIntgrPts	
element	forceBeamColumn	40041	740008	416	$TransfTag	Lobatto	$W14x176	$numIntgrPts	
element	forceBeamColumn	40051	750008	516	$TransfTag	Lobatto	$W14x159	$numIntgrPts	
									
element	forceBeamColumn	40012	117	126	$TransfTag	Lobatto	$W14x159	$numIntgrPts	
element	forceBeamColumn	40022	217	226	$TransfTag	Lobatto	$W14x176	$numIntgrPts	
element	forceBeamColumn	40032	317	326	$TransfTag	Lobatto	$W14x176	$numIntgrPts	
element	forceBeamColumn	40042	417	426	$TransfTag	Lobatto	$W14x176	$numIntgrPts	
element	forceBeamColumn	40052	517	526	$TransfTag	Lobatto	$W14x159	$numIntgrPts	
									
element	forceBeamColumn	40013	127	136	$TransfTag	Lobatto	$W14x159	$numIntgrPts	
element	forceBeamColumn	40023	227	236	$TransfTag	Lobatto	$W14x176	$numIntgrPts	
element	forceBeamColumn	40033	327	336	$TransfTag	Lobatto	$W14x176	$numIntgrPts	
element	forceBeamColumn	40043	427	436	$TransfTag	Lobatto	$W14x176	$numIntgrPts	
element	forceBeamColumn	40053	527	536	$TransfTag	Lobatto	$W14x159	$numIntgrPts	
									
#Consolidated Frame									
element	forceBeamColumn	40061	760008	616	$TransfTag	Lobatto	$W14x176_CF1_WA	$numIntgrPts	
element	forceBeamColumn	40071	770008	716	$TransfTag	Lobatto	$W10x77_CF	$numIntgrPts	
element	forceBeamColumn	40081	780008	816	$TransfTag	Lobatto	$W10x77_CF	$numIntgrPts	
element	forceBeamColumn	40091	790008	916	$TransfTag	Lobatto	$W10x77_CF	$numIntgrPts	
element	forceBeamColumn	400101	7100008	1016	$TransfTag	Lobatto	$W14x176_CF1_WA	$numIntgrPts	
									
element	forceBeamColumn	40062	617	626	$TransfTag	Lobatto	$W14x176_CF2_WA	$numIntgrPts	
element	forceBeamColumn	40072	717	726	$TransfTag	Lobatto	$W10x60_CF	$numIntgrPts	
element	forceBeamColumn	40082	817	826	$TransfTag	Lobatto	$W10x60_CF	$numIntgrPts	
element	forceBeamColumn	40092	917	926	$TransfTag	Lobatto	$W10x60_CF	$numIntgrPts	
element	forceBeamColumn	400102	1017	1026	$TransfTag	Lobatto	$W14x176_CF2_WA	$numIntgrPts	
									
element	forceBeamColumn	40063	627	636	$TransfTag	Lobatto	$W14x176_CF2_WA	$numIntgrPts	
element	forceBeamColumn	40073	727	736	$TransfTag	Lobatto	$W10x60_CF	$numIntgrPts	
element	forceBeamColumn	40083	827	836	$TransfTag	Lobatto	$W10x60_CF	$numIntgrPts	
element	forceBeamColumn	40093	927	936	$TransfTag	Lobatto	$W10x60_CF	$numIntgrPts	
element	forceBeamColumn	400103	1027	1036	$TransfTag	Lobatto	$W14x176_CF2_WA	$numIntgrPts	


#beams
#Moment Resisting Frame												
element	forceBeamColumn	60011	1105	2110	$TransfTag	HingeRadau	$W24x76	$Lp_Beam	$W24x76	$Lp_Beam	[expr	500*$W24x76]
element	forceBeamColumn	60021	2105	3110	$TransfTag	HingeRadau	$W24x76	$Lp_Beam	$W24x76	$Lp_Beam	[expr	500*$W24x76]
element	forceBeamColumn	60031	3105	4110	$TransfTag	HingeRadau	$W24x76	$Lp_Beam	$W24x76	$Lp_Beam	[expr	500*$W24x76]
element	forceBeamColumn	60041	4105	5110	$TransfTag	HingeRadau	$W24x76	$Lp_Beam	$W24x76	$Lp_Beam	[expr	500*$W24x76]
												
element	forceBeamColumn	60012	1205	2210	$TransfTag	HingeRadau	$W24x84	$Lp_Beam	$W24x84	$Lp_Beam	[expr	500*$W24x84]
element	forceBeamColumn	60022	2205	3210	$TransfTag	HingeRadau	$W24x84	$Lp_Beam	$W24x84	$Lp_Beam	[expr	500*$W24x84]
element	forceBeamColumn	60032	3205	4210	$TransfTag	HingeRadau	$W24x84	$Lp_Beam	$W24x84	$Lp_Beam	[expr	500*$W24x84]
element	forceBeamColumn	60042	4205	5210	$TransfTag	HingeRadau	$W24x84	$Lp_Beam	$W24x84	$Lp_Beam	[expr	500*$W24x84]
												
element	forceBeamColumn	60013	1305	2310	$TransfTag	HingeRadau	$W18x40	$Lp_Beam	$W18x40	$Lp_Beam	[expr	500*$W18x40]
element	forceBeamColumn	60023	2305	3310	$TransfTag	HingeRadau	$W18x40	$Lp_Beam	$W18x40	$Lp_Beam	[expr	500*$W18x40]
element	forceBeamColumn	60033	3305	4310	$TransfTag	HingeRadau	$W18x40	$Lp_Beam	$W18x40	$Lp_Beam	[expr	500*$W18x40]
element	forceBeamColumn	60043	4305	5310	$TransfTag	HingeRadau	$W18x40	$Lp_Beam	$W18x40	$Lp_Beam	[expr	500*$W18x40]
												
#Consolidated Frame					$A $E $Iz $transfTag 							
element	elasticBeamColumn 	60061	6111	7111	0.012387072	$Es	0.000619128	$TransfTag				
element	elasticBeamColumn 	60071	7112	8111	0.012387072	$Es	0.000619128	$TransfTag				
element	elasticBeamColumn 	60081	8112	9111	0.012387072	$Es	0.000619128	$TransfTag				
element	elasticBeamColumn 	60091	9112	10111	0.012387072	$Es	0.000619128	$TransfTag				
												
element	elasticBeamColumn 	60062	6211	7211	0.012387072	$Es	0.000619128	$TransfTag				
element	elasticBeamColumn 	60072	7212	8211	0.012387072	$Es	0.000619128	$TransfTag				
element	elasticBeamColumn 	60082	8212	9211	0.012387072	$Es	0.000619128	$TransfTag				
element	elasticBeamColumn 	60092	9212	10211	0.012387072	$Es	0.000619128	$TransfTag				
												
element	elasticBeamColumn 	60063	6311	7311	0.010467721	$Es	0.00042529	$TransfTag				
element	elasticBeamColumn 	60073	7312	8311	0.010467721	$Es	0.00042529	$TransfTag				
element	elasticBeamColumn 	60083	8312	9311	0.010467721	$Es	0.00042529	$TransfTag				
element	elasticBeamColumn 	60093	9312	10311	0.010467721	$Es	0.00042529	$TransfTag				
...
uniaxialMaterial ElasticPP $SC16x26_CF [expr 0.4*$My/0.02] 0.02 -0.01 ;

set SC14x22_CF 222;
set My [expr 0.00136*$Fy_beam];
uniaxialMaterial ElasticPP $SC14x22_CF [expr 0.4*$My/0.02] 0.02 -0.01 ;

#definir elementos e constraints
element	zeroLength	761006	6105	6111	-mat	$SC16x26_CF	-dir	6
element	zeroLength	762006	7110	7111	-mat	$SC16x26_CF	-dir	6
element	zeroLength	763006	6205	6211	-mat	$SC16x26_CF	-dir	6
element	zeroLength	771004	7210	7211	-mat	$SC16x26_CF	-dir	6
element	zeroLength	772004	6305	6311	-mat	$SC14x22_CF	-dir	6
element	zeroLength	773004	7310	7311	-mat	$SC14x22_CF	-dir	6
								
element	zeroLength	771006	7105	7112	-mat	$SC16x26_CF	-dir	6
element	zeroLength	772006	8110	8111	-mat	$SC16x26_CF	-dir	6
element	zeroLength	773006	7205	7212	-mat	$SC16x26_CF	-dir	6
element	zeroLength	781004	8210	8211	-mat	$SC16x26_CF	-dir	6
element	zeroLength	782004	7305	7312	-mat	$SC14x22_CF	-dir	6
element	zeroLength	783004	8310	8311	-mat	$SC14x22_CF	-dir	6
								
element	zeroLength	781006	8105	8112	-mat	$SC16x26_CF	-dir	6
element	zeroLength	782006	9110	9111	-mat	$SC16x26_CF	-dir	6
element	zeroLength	783006	8205	8212	-mat	$SC16x26_CF	-dir	6
element	zeroLength	791004	9210	9211	-mat	$SC16x26_CF	-dir	6
element	zeroLength	792004	8305	8312	-mat	$SC14x22_CF	-dir	6
element	zeroLength	793004	9310	9311	-mat	$SC14x22_CF	-dir	6
								
element	zeroLength	791006	9105	9112	-mat	$SC16x26_CF	-dir	6
element	zeroLength	792006	10110	10111	-mat	$SC16x26_CF	-dir	6
element	zeroLength	793006	9205	9212	-mat	$SC16x26_CF	-dir	6
element	zeroLength	7101004	10210	10211	-mat	$SC16x26_CF	-dir	6
element	zeroLength	7102004	9305	9312	-mat	$SC14x22_CF	-dir	6
element	zeroLength	7103004	10310	10311	-mat	$SC14x22_CF	-dir	6
...
elemPanelZone2D	500111	1101	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500211	2101	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500311	3101	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500411	4101	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500511	5101	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500611	6101	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500711	7101	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500811	8101	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500911	9101	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	5001011	10101	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
							
elemPanelZone2D	500121	1201	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500221	2201	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500321	3201	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500421	4201	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500521	5201	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500621	6201	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500721	7201	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500821	8201	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500921	9201	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	5001021	10201	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
							
elemPanelZone2D	500131	1301	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500231	2301	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500331	3301	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500431	4301	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500531	5301	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500631	6301	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500731	7301	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500831	8301	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	500931	9301	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
elemPanelZone2D	5001031	10301	[expr 1.0E5*$GPa]	1	0.1	$TransfTag;	
...
rotPanelZone2D	41100	1103	1104	$Es	$Fy_beam	0.380492	0.395351	0.030226	0.018923	0.607568	1.2	0.03;
rotPanelZone2D	42100	2103	2104	$Es	$Fy_beam	0.386588	0.39751	0.033274	0.027432	0.607568	1.2	0.03;
rotPanelZone2D	43100	3103	3104	$Es	$Fy_beam	0.386588	0.39751	0.033274	0.027432	0.607568	1.2	0.03;
rotPanelZone2D	44100	4103	4104	$Es	$Fy_beam	0.386588	0.39751	0.033274	0.027432	0.607568	1.2	0.03;
rotPanelZone2D	45100	5103	5104	$Es	$Fy_beam	0.380492	0.395351	0.030226	0.018923	0.607568	1.2	0.03;
rotPanelZone2D	46100	6103	6104	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.386588	0.39751	0.033274	0.021082	0.398526	1.2	0.03;
rotPanelZone2D	47100	7103	7104	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.26924	0.258826	0.022098	0.019812	0.398526	1.2	0.03;
rotPanelZone2D	48100	8103	8104	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.26924	0.258826	0.022098	0.019812	0.398526	1.2	0.03;
rotPanelZone2D	49100	9103	9104	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.26924	0.258826	0.022098	0.019812	0.398526	1.2	0.03;
rotPanelZone2D	410100	10103	10104	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.386588	0.39751	0.033274	0.021082	0.398526	1.2	0.03;
												
rotPanelZone2D	41200	1203	1204	$Es	$Fy_beam	0.380492	0.395351	0.030226	0.018923	0.61214	1.2	0.03;
rotPanelZone2D	42200	2203	2204	$Es	$Fy_beam	0.386588	0.39751	0.033274	0.035372	0.61214	1.2	0.03;
rotPanelZone2D	43200	3203	3204	$Es	$Fy_beam	0.386588	0.39751	0.033274	0.035372	0.61214	1.2	0.03;
rotPanelZone2D	44200	4203	4204	$Es	$Fy_beam	0.386588	0.39751	0.033274	0.035372	0.61214	1.2	0.03;
rotPanelZone2D	45200	5203	5204	$Es	$Fy_beam	0.380492	0.395351	0.030226	0.018923	0.61214	1.2	0.03;
rotPanelZone2D	46200	6203	6204	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.386588	0.39751	0.033274	0.021082	0.398526	1.2	0.03;
rotPanelZone2D	47200	7203	7204	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.259588	0.256032	0.017272	0.024958	0.398526	1.2	0.03;
rotPanelZone2D	48200	8203	8204	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.259588	0.256032	0.017272	0.024958	0.398526	1.2	0.03;
rotPanelZone2D	49200	9203	9204	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.259588	0.256032	0.017272	0.024958	0.398526	1.2	0.03;
rotPanelZone2D	410200	10203	10204	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.386588	0.39751	0.033274	0.021082	0.398526	1.2	0.03;
												
rotPanelZone2D	41300	1303	1304	$Es	$Fy_beam	0.380492	0.395351	0.030226	0.018923	0.45466	1.2	0.03;
rotPanelZone2D	42300	2303	2304	$Es	$Fy_beam	0.386588	0.39751	0.033274	0.021082	0.45466	1.2	0.03;
rotPanelZone2D	43300	3303	3304	$Es	$Fy_beam	0.386588	0.39751	0.033274	0.021082	0.45466	1.2	0.03;
rotPanelZone2D	44300	4303	4304	$Es	$Fy_beam	0.386588	0.39751	0.033274	0.021082	0.45466	1.2	0.03;
rotPanelZone2D	45300	5303	5304	$Es	$Fy_beam	0.380492	0.395351	0.030226	0.018923	0.45466	1.2	0.03;
rotPanelZone2D	46300	6303	6304	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.386588	0.39751	0.033274	0.021082	0.348996	1.2	0.03;
rotPanelZone2D	47300	7303	7304	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.259588	0.256032	0.017272	0.010668	0.348996	1.2	0.03;
rotPanelZone2D	48300	8303	8304	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.259588	0.256032	0.017272	0.010668	0.348996	1.2	0.03;
rotPanelZone2D	49300	9303	9304	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.259588	0.256032	0.017272	0.010668	0.348996	1.2	0.03;
rotPanelZone2D	410300	10303	10304	[expr $Es*2.5]	[expr $Fy_beam*2.5]	0.386588	0.39751	0.033274	0.021082	0.348996	1.2	0.03;
...
rayleigh $alphaM $betaKcurr $betaKinit $betaKcomm; # RAYLEIGH damping
	...
	set TmaxAnalysis	[expr $npts_accelerogram*$dt_accelerogram*$sec + 0.0*$sec];	# maximum duration of ground-motion analysis 
